...
클라이언트와 서버 간의 연결을 `Session`으로 관리하려고 할때 세션의 생존과 해제 조건이 명확하지 않다는 문제가 생겼고, 세션이 살아 있다는게 여러 가지 의미로 해석될 수 있어 세션 생존을 단일 소유권으로 관리하기 어려웠습니다. 그래서 세션 수명 관리 장치를 하나의 소유권으로 합치는 것이 아니라, 각 장치가 보장하는 방식으로 수명 관리를 설계했습니다.
...
따라서 콜백 대상 생존은 `SessionManager`가 아니라 제출된 operation 객체가 보장합니다. `SessionManager`는 현재 연결 목록을 관리하고, 제출 이벤트는 이미 커널에 제출된 I/O가 완료될 때까지 콜백 대상을 살려두는 책임을 가집니다.
...
따라서 `pending_io_`는 이미 제출한 I/O가 모두 끝났는지를 판단하고, `SessionState`는 지금 새 작업을 받아도 되는지를 판단합니다. 둘 중 하나만으로는 안전한 종료 조건을 만들 수 없습니다.
...
```
`SessionManager`는 외부에 노출되는 연결 목록을 관리하고, `OwnerBoundEvent`는 CQE 콜백 대상의 생존을 보장하며, `DispatchResult`는 이벤트를 완료할지 유지할지 재사용할지 결정합니다. 여기에 `pending_io_`가 최종 CQE 대기 수를 추적하고, `SessionState`가 새 작업 허용 여부와 종료 단계를 막아 줍니다. 이렇게 여러 장치가 각자 보장하는 방식으로 수명 관리를 설계해, 세션이 수명을 관리하는 여러 조건을 안전하게 충족할 수 있도록 했습니다.