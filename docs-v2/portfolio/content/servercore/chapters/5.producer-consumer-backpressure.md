...
초기 송신 큐 방어는 주로 몇 개의 버퍼가 쌓였는지를 기준으로 삼았습니다. 하지만 큐 항목 수만으로는 실제 메모리 압력을 알 수 없었고, 같은 10개라도 작은 패킷 10개와 256 KiB body chunk 10개는 다른 상황이었습니다.
...
여기서 `RecvBuffer`와 `PausedRecvQueue`는 다른 책임을 가집니다. `RecvBuffer`는 프로토콜 프레임이 조각났을 때 패킷을 조립하기 위한 세션 내부 버퍼이고, `PausedRecvQueue`는 백프레셔 때문에 `OnRecv()` 전달을 늦추는 queue-owned bounded 큐 입니다.
...
이 장치의 목적은 스트림 보존이 목적입니다. write-side 압력이 read-side pause로 전파되더라도, 이미 도착한 바이트는 순서를 유지한 채 제한된 범위에서만 살아남습니다.
...
작은 응답은 header와 body를 하나의 send buffer로 만들어 바로 `Send()`할 수 있지만, 큰 응답을 한 번에 만들면 느린 클라이언트 앞에서 send queue와 buffer pool에 큰 압력이 생깁니다. 그래서 큰 body는 먼저 header만 전송하고, body는 `BodyStreamState`에 보관한 뒤 일정 크기의 chunk로 나누어 공급합니다.
