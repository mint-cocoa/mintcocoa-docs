...
게임 서버에서 `Room`은 같은 공간, 채널, 전투 인스턴스처럼 함께 상태를 공유하는 최소 단위입니다. 플레이어 입장, 퇴장, 이동, 공격, 포탈 이동, 브로드캐스트 대상 결정처럼 누가 같은 공간에 있고, 어떤 순서로 상태가 바뀌었는지를 판단해야 하는 작업은 Room의 상태를 기준으로 처리됩니다.
...
Room은 게임 상태를 순서대로 바꾸는 곳이고, Session은 한 연결에 바이트를 순서대로 쓰는 곳입니다. Room의 브로드캐스트 결과는 Session 송신 큐에 들어가는 입력일 뿐이며, 실제 커널 송신 제출과 부분 송신 복구는 Session의 책임입니다.
...
단순하게 모든 상태를 뮤텍스로 보호할 수도 있었지만 그렇게 하면 게임 로직, 소켓 I/O, Room 브로드캐스트, Session 송신 경로 전체에 락이 퍼져 어떤 상태를 누가 소유하는지 불명확해지고, 락 순서를 잘못 잡아 데드락이 발생할 위험도 커집니다.

그래서 전역 락 대신 상태의 소유자를 명확히 나누고, 스레드 간 작업을 해당 상태의 소유자 실행 context로 모으는 방식을 선택했습니다.
...
`WorkerOutbox`는 세션 id별 큐를 가지고, `IoWorker::Run()`의 drain 단계에서 소유 스레드가 직접 비웁니다. 즉시 `Send()`하지 않고 반드시 outbox를 거치기 때문에, 같은 세션으로 가는 Room 출력은 하나의 큐 경로를 통과합니다.
...
`Room JobQueue`는 Room 상태 변경과 브로드캐스트 결정을 한 번에 하나씩 처리하고, `WorkerOutbox`는 Room 출력이 대상 워커로 이동할 때 세션별 FIFO를 유지하며, `Session owner ring`은 같은 세션의 `SendQueue` 삽입과 `SendOp` 진행을 한 실행 문맥에서 처리합니다.
