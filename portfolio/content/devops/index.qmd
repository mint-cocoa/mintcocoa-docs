---
title: "오라클 클라우드 기반 Kubernetes 프로덕션 인프라 아키텍처"
author: "배진후"
---
...
그래서 오라클 클라우드의 프리 티어 OKE 클러스터를 이용해 프로덕션과 개발 환경을 분리한다면
...
public-facing 리소스는 Internet Gateway 경로를 사용하고, pod outbound traffic은 NAT Gateway 경로를 사용하도록 해
public ingress와 private API, pod outbound traffic의 경로를 분리했습니다.


![OCI routing Terraform capture](assets/production-infrastructure/gitops/fig-oci-oke-vcn-routing-boundary.svg)

VCN은 역할별 subnet으로 나누어집니다. endpoint subnet은 Kubernetes API endpoint를 위한 공간이고,
pods subnet은 OCI VCN native CNI가 자동으로 pod address를 할당하는 영역이므로 별도 CNI를 둘 필요가 없었습니다.
 management subnet은 집 내부망에서 IPSec 경로로 접근하는 management k3s/Argo CD GitOps manager VM 영역입니다.
...
![pipeline-demo staging image update PR](assets/production-infrastructure/gitops/gitops-pipeline-demo-staging-pr.png)
...
![GitHub Actions promotion workflow runs](assets/production-infrastructure/gitops/gitops-promotion-workflow-runs.png)

수동 실행 화면에서는 앱 이름만 입력합니다. workflow는 입력받은 앱의 staging/prod overlay 경로를 계산하고, staging에 기록된 image tag를 prod overlay로 복사합니다.

![GitHub Actions promotion workflow input](assets/production-infrastructure/gitops/gitops-promotion-workflow-dispatch-input.png)
...
![pipeline-demo production promotion PR](assets/production-infrastructure/gitops/gitops-pipeline-demo-prod-promotion-pr.png)
...
![Argo CD prod-pipeline-demo OutOfSync before manual sync](assets/production-infrastructure/gitops/gitops-pipeline-demo-argocd-outofsync.png)
...
3. `alert-ai-webhook`은 `대상, 증상, 시간 정보만 추출해 AI에게 전달하는 1차 프롬프트`로 AI에게 알림 사실을 전달합니다. 
