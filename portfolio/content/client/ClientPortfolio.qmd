...
| **서버** | [iouring-runtime/examples/game/dungeon_full_server](https://github.com/mint-cocoa/iouring-runtime/tree/main/examples/game/dungeon_full_server) on `RuntimeGame` |
| **공개 저장소** | [game-client](https://github.com/mint-cocoa/game-client) |
...
DirectX 11 API로 제작한 멀티플레이 던전 RPG 클라이언트입니다. 서버 쪽 기준 구현은 `iouring-runtime`의 `RuntimeGame` 위에 올라간 `dungeon_full_server` 예제입니다. 클라이언트는 DX11 파이프라인, 리소스 로더, 네트워크 계층을 별도로 두고 TCP/Protobuf 패킷 흐름과 렌더링 구조를 정리했습니다.
```
isometric_client/
├── src/
...
: 벽 충돌 확인 레이어 {#tbl-wall-collision}
...
| HandleMove 벽 충돌 거부 | DungeonGenerator.GetTile() | 서버 권위적 이동 확인 |
...
서버 아키텍처 상세(io_uring 네트워크 엔진, JobQueue 공정 스케줄링, BufferPool, 벤치마크 등)는 [ServerCore 포트폴리오](/portfolio/servercore/chapters/01-overview.html)를 참조하시기 바랍니다.
...
| [iouring-runtime](https://github.com/mint-cocoa/iouring-runtime) | `RuntimeGame`과 `dungeon_full_server` 기준 서버 |
| [multiplayer-dungeon-rpg-server](https://github.com/mint-cocoa/multiplayer-dungeon-rpg-server) | 통합 이전 연동 게임 서버 히스토리 |
| [libiouring-core](https://github.com/mint-cocoa/libiouring-core) | 통합 이전 서버 런타임 히스토리 |
:::

## 다음 문서

::: {.next-docs}
[포트폴리오 문서 인덱스](/portfolio/) ·
[서버 구현 포트폴리오](/portfolio/servercore/chapters/01-overview.html) ·
[DevOps 운영 문서](/portfolio/devops/)
:::
